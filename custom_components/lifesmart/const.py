...
8. 【设备聚合列表(ALL_*_TYPES)】
   用于快速判断设备所属平台，避免重复的设备类型检查：
   - ALL_SWITCH_TYPES：所有开关类设备的并集
   - ALL_LIGHT_TYPES：所有灯光类设备的并集
   - ALL_SENSOR_TYPES：所有传感器类设备的并集
   - 这些列表在__init__.py的get_platform_by_device()中使用
...
# ================= 插座系列 (Outlet Series) =================
# --- 传统插座系列 (Traditional Outlets) ---
TRADITIONAL_OUTLET_TYPES = {
    # 智慧插座 SL_OL
    # IO口: O(开关 RW)
    #   数据类型：整数开关状态
    ...
    "SL_OL",
    # 智慧插座(3C版) SL_OL_3C - 同SL_OL功能
    "SL_OL_3C",
    # 德标插座 SL_OL_DE - 同SL_OL功能
    "SL_OL_DE",
    # 英标插座 SL_OL_UK - 同SL_OL功能
    "SL_OL_UK",
    # 美标插座 SL_OL_UL - 同SL_OL功能
    "SL_OL_UL",
    # Wi-Fi插座 OD_WE_OT1
    # IO口: P1(开关 RW)
    #   数据类型：整数开关状态
    ...
    #   下发命令：打开type=0x81,val=1; 关闭type=0x80,val=0
    "OD_WE_OT1",
    # 入墙插座 SL_OL_W - 兼具开关功能的插座
    # IO口: L1(开关 RW) dark(关状态时指示灯亮度 RW) bright(开状态时指示灯亮度 RW)
    #   功能同流光开关SL_SW_IF1，参考SUPPORTED_SWITCH_TYPES中的详细定义
    "SL_OL_W",
}

# --- 计量插座系列 (Metering Outlets) ---
METERING_OUTLET_TYPES = {
    ...
    "SL_OE_3C",
    # 计量插座德标 SL_OE_DE - 同SL_OE_3C功能
    "SL_OE_DE",
    # 计量插座入墙版 SL_OE_W - 同SL_OE_3C功能
    "SL_OE_W",
}
...
# --- 随心开关和按键系列 ---
CUBE_BUTTON_TYPES = {
    # 随心开关(CUBE Clicker) SL_SC_BB
    # IO口: V(电量 R) B(按键状态 R)
    #
    # V(电量 R)
    #   数据类型：整数电压值和百分比
    #   属性值描述：val值表示原始电压值, v值将表示当前剩余电量百分比，值范围[0,100]，根据val电压值换算
    #   电池管理：低电时需要更换电池
    #
    # B(按键状态 R)
    #   数据类型：整数按键状态
    #   属性值描述：val的值定义如下: 0未按下按键, 1按下按键
    #   触发机制：物理按键按下时置1，松开时置0
    "SL_SC_BB",
    # 随心按键(CUBE Clicker2) SL_SC_BB_V2 - 高级版本支持复杂手势
    # IO口: P1(按键状态 R) P2(电量 R)
    #
    # P1(按键状态 R)
    #   数据类型：整数按键事件类型
    #   属性值描述：type的值定义如下: type&1==1表示有按键事件产生; type&1==0表示按键事件消失
    #              val值指明按键事件类型，只有在type&1==1时才有效，val值定义:
    #              1单击事件, 2双击事件, 255长按事件
    #   触发机制：检测到手势后产生对应事件，事件持续一定时间后自动消失
    #
    # P2(电量 R)
    #   数据类型：整数电压值和百分比
    #   属性值描述：val值表示原始电压值, v值将表示当前剩余电量百分比，值范围[0,100]，根据val电压值换算
...
VOLTAGE_SWITCH_TYPES = {
    ...
    # P2(电量 R)
    #   数据类型：整数电压值和百分比
    #   属性值描述：val值表示原始电压值, v值将表示当前剩余电量百分比，值范围[0,100]
    #   电池管理：它是根据val电压值换算的电量百分比，低电时需要更换电池
    #   版本信息：极星开关系列其fullCls的版本号为V2，例如SL_SW_ND1_V2指明其为极星1联开关
    "SL_SW_ND1",
    # 恒星/辰星/极星开关伴侣(单键)
    # IO口: P1(开关 RW) - 第一路开关控制口
    # P2(电量 R) - 原始电压值,v为电量百分比[0,100]
    "SL_MC_ND1",
...
SMART_PLUG_TYPES = {
    # === 传统智慧插座系列 (Traditional Smart Plugs) ===
    # 智慧插座
    # IO口: O(开关 RW)
    #   属性值描述: type&1==1表示打开(忽略val值); type&1==0表示关闭(忽略val值)
    #   下发命令: 打开type=0x81,val=1; 关闭type=0x80,val=0
    ...
    #   下发命令: 打开type=0x81,val=1; 关闭type=0x80,val=0
    # dark(关状态指示灯亮度 RW)
    #   属性值描述: type&1==1表示打开; type&1==0表示关闭; val表示指示灯亮度值
    #   亮度定义: bit0~bit7:Blue, bit8~bit15:Green, bit16~bit23:Red, bit24~bit31:white
    #   当White>0时表示动态模式,具体参考动态颜色(DYN)定义
    #   下发命令: 开灯type=0x81,val=1; 关灯type=0x80,val=0;
    #            开灯并设置颜色或动态值type=0xff,val=颜色或动态值
    #            关灯并设置颜色值或动态值type=0xfe,val=颜色或动态值
    # bright(开状态指示灯亮度 RW) - 同dark定义
    "SL_OL_W",
    # Wi-Fi插座 (OD_WE_OT1)
    # IO口: P1(开关 RW)
    #   属性值描述: type&1==1表示打开(忽略val值); type&1==0表示关闭(忽略val值)
...
POWER_METER_PLUG_TYPES = {
    ...
    #   下发命令: 打开type=0x81,val=1; 关闭type=0x80,val=0
    # P2(用电量 R) - 累计用电量
    #   属性值描述: val值为IEEE754浮点数的32位整数表示,v值为浮点数,单位度(kwh)
    #   注意: v值可以直接使用,若不存在v值则需要手动转换
    #        其值类型为IEEE 754浮点数的32位整数布局
    #        例如:1024913643表示的是浮点值:0.03685085
    #        可以依据整数值计算出浮点值,具体参考附录3.4 IO值浮点类型说明
    # P3(功率 R) - 当前负载功率
    #   属性值描述: v值为浮点数,单位w
    #   注意: v值可以直接使用,若不存在v值则需要手动转换
    #        其值类型为IEEE 754浮点数的32位整数布局,转换方法同P2
    # P4(功率门限 RW) - 功率门限用电保护
    #   属性值描述: 值为整数,超出门限则会断电,单位w
    #   下发命令: 使能type=0x81,val=1; 不使能type=0x80,val=0
    #            使能并设置门限type=207,val=门限值
    #            不使能并设置门限type=206,val=门限值
    "SL_OE_DE",
    # 计量插座(国标3C) SL_OE_3C - 同SL_OE_DE规范
    # IO口: P1(开关 RW) P2(用电量 R) P3(功率 R) P4(功率门限 RW)
    #   IO口功能定义完全同SL_OE_DE,参考上述详细说明
    "SL_OE_3C",
    # 入墙计量插座 SL_OE_W - 同SL_OE_DE规范
    # IO口: P1(开关 RW) P2(用电量 R) P3(功率 R) P4(功率门限 RW)
    #   IO口功能定义完全同SL_OE_DE,参考上述详细说明
    "SL_OE_W",
}
...
BASIC_ENV_SENSOR_TYPES = {
    ...
    # H(当前环境湿度 R) - 湿度检测传感器
    #   数据类型：整数湿度值
    #   属性值描述：val为原始湿度值*10,v为实际湿度值,单位:%RH
    #   测量范围：0-100%RH,精度±3%RH
    #   传感器类型：电容式湿度传感器,集成在温湿度传感器中
    #   数据处理：val需要除以10得到实际湿度,如val=650表示65.0%RH
    #
    # Z(当前环境光照 R) - 光照强度检测
    #   数据类型：整数光照值
...
# --- 总开关列表 ---
# 基于IO口功能的精确分类，只包含实际具有开关控制IO口的设备
ALL_SWITCH_TYPES = (
    # 专门的开关设备
    SUPPORTED_SWITCH_TYPES  # 各种面板开关
    | VOLTAGE_SWITCH_TYPES  # 带电压监测的开关
    | SMART_PLUG_TYPES  # O口开关控制
    | POWER_METER_PLUG_TYPES  # P1/P4开关控制
    # 动态分类设备(根据工作模式决定是否为开关)
    | GENERIC_CONTROLLER_TYPES  # 开关模式时P2/P3/P4为开关控制
    # 485控制器可能包含开关功能(Lx口)
    | CONTROLLER_485_TYPES
)
...
# --- 总二元传感器列表 ---
# 基于IO口功能的精确分类，只包含实际具有二元状态IO口的设备
ALL_BINARY_SENSOR_TYPES = (
    # 专门的二元传感器设备
    BINARY_SENSOR_TYPES  # 门窗感应器、动态感应器
    | WATER_SENSOR_TYPES  # WA导电率检测
    | SMOKE_SENSOR_TYPES  # P1烟雾告警检测
    | RADAR_SENSOR_TYPES  # P1人体存在检测
    | DEFED_SENSOR_TYPES  # 云防系列各种检测
    # 附带二元传感器功能的设备
    | LOCK_TYPES  # EVTLO解锁事件, ALM报警事件
    | CLIMATE_TYPES  # 温控设备的阀门/继电器状态(P2/P3/P5等)
    | BUTTON_SWITCH_TYPES  # B/P1按键状态检测
    # 动态分类设备(根据工作模式决定是否为二元传感器)
    | GENERIC_CONTROLLER_TYPES  # 自由模式时P5/P6/P7为二元传感器输入
)

# --- 总数值传感器列表 ---
# 基于IO口功能的精确分类，只包含实际具有数值传感器IO口的设备
ALL_SENSOR_TYPES = (
    # 专门的传感器设备
    EV_SENSOR_TYPES
    | GAS_SENSOR_TYPES
    ...
    | POWER_METER_TYPES
    | VOICE_SENSOR_TYPES
    | AIR_PURIFIER_TYPES  # 有温湿度PM2.5等传感器IO口
    | CONTROLLER_485_TYPES  # 485控制器可接入各种传感器
    # 附带电量/状态传感器的设备
    | LOCK_TYPES  # BAT电量传感器
    | COVER_TYPES  # P8位置传感器,电量传感器
    | BUTTON_SWITCH_TYPES  # V/P2电量传感器
    | SMOKE_SENSOR_TYPES  # P2电压传感器
    | WATER_SENSOR_TYPES  # V电量传感器
    | POWER_METER_PLUG_TYPES  # P2/P3/P4功率电量传感器
    # 特殊设备: 超能面板温控版的P4温度传感器由helpers.py逻辑处理
)
...
CMD_TYPE_SET_RAW = 0xFF  # 开灯亮度设置(颜色、动态等)
CMD_TYPE_BRIGHTNESS_OFF = 0xFE  # 关灯亮度设置（颜色、动态等）
